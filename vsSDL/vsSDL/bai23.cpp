...
int main(int argc, char* args[]) {
	...
	if (loadMedia() == false) {
		...
		gStartTexture.loadFromFile("Press S to Start or Stop the Timer", textColor);
		gPauseTexture.loadFromFile("Press P to Pause or Unpause the Timer", textColor);
		while (quit == false) {
			...
			timeText.str("");
			timeText << "123: " << (gTimer.getTicks());
			
			gTimeTexture.loadFromFile(timeText.str().c_str(), textColor);

			SDL_RenderClear(gRenderer);

			gStartTexture.render((SCREEN_WIDTH, gStartTexture.getWidth()) / 2, 0);
			gPauseTexture.render((SCREEN_WIDTH - gPauseTexture.getWidth()) / 2, gStartTexture.getHeight());
